                ...
                <tbody>
                    ...
                    foreach ($bank_titipan as $bt) {
                        ...
                        foreach ($tutup_jurnal as $jurnal) {
                            $tgl_input_jurnal = date("m-Y", strtotime($jurnal->tgl_jurnal.'+1 month'));
...
foreach ($tutup_jurnal as $jurnalku) {
    $tgl_jurnalku = date("m-Y", strtotime($jurnalku->tgl_jurnal));
    ...
    if (isset($_GET['dari']) && isset($_GET['ke'])) {
        ...
        if ($tgl_jurnalku == $tgl_dari || $tgl_jurnalku == $tgl_ke) {
            $hapus_jurnal = true; 
            $id_jurnalku = $jurnalku->id_jurnal;
        }
    ...
    if ($tgl_jurnalku == $waktu) {
        $hapus_jurnal = true;
        $id_jurnalku = $jurnalku->id_jurnal;
        $bulan = date("F", strtotime($waktu_awal));
    }

}

?>
<?php if ($hapus_jurnal && !$skip_blocks) { ?>
    <div class="text-right mt-3">
        <a href="<?= base_url('BankTitipan/hapus_tutup_jurnal?id_jurnal=' . $id_jurnalku); ?>" onclick="javascript:return confirm('Apakah Anda yakin ingin hapus jurnal bulan (<?= $bulan ?>) ?')" class="btn btn-danger mt-1">
            <i class="fas fa-trash" title="Hapus Jurnal"></i>  Hapus Jurnal (<?= $bulan ?>)
        </a>
    </div>
<?php } elseif (!$skip_blocks) {
    $bulanku = isset($_GET['dari']) && isset($_GET['ke']) ? $bulan : $nama_bulan;
    ?>
    <div class="text-right mt-3">
        <a href="<?= base_url('BankTitipan/tutup_jurnal?tsa=' . $tsa.'&dari='.$b. '&ke='.$t.'&bulan='.$bulanku); ?>"
